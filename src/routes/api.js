...
function cleanPostContent(content) {
  return sanitizeHtml(content || '', {
    allowedTags: sanitizeHtml.defaults.allowedTags.concat(['img', 'figure', 'h1', 'h2', 'h3', 'iframe', 'div', 'span']),
    allowedAttributes: {
...
router.get('/admin/posts/:id', requireAuth, async (req, res) => {
  try {
    const post = await Post.findByPk(req.params.id, { include: [{ model: Category, as: 'category' }] });
