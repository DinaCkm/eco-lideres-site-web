...
<div class="layout">
  ...
  <div class="editor-col">
    ...
    <div class="field">
      ...
      <div class="help-text">Para inserir ou editar imagem no texto: clique no ícone de imagem ou clique duas vezes em uma imagem já inserida. Use "Incorporar / Embed" para YouTube, Vimeo, Typeform, Google Forms e outros iframes. Quando usar "Editar HTML", a prévia HTML real aparece nesta área.</div>
    </div>
  </div>
...
async function save(forceStatus) {
  ...
  if (rawHtmlMode) {
    content = removeBase64Images(rawHtmlContent || document.getElementById('htmlPreview').innerHTML);
  } else {
    normalizeExistingImages();
    content = removeBase64Images(quill.root.innerHTML);
  }
...
function insertEmbedFromModal() {
  ...
  if (rawHtmlMode) {
    rawHtmlContent = (rawHtmlContent || document.getElementById('htmlPreview').innerHTML) + iframeHtml + '<p><br></p>';
    document.getElementById('htmlPreview').innerHTML = rawHtmlContent;
    closeEmbedModal();
    return;
  }
...
function openHtmlModal() {
  if (rawHtmlMode) {
    document.getElementById('htmlEditorTextarea').value = rawHtmlContent || document.getElementById('htmlPreview').innerHTML;
  } else {
    normalizeExistingImages();
...
function applyHtmlFromModal() {
  ...
  rawHtmlMode = true;
  rawHtmlContent = removeBase64Images(html);
  ...
  htmlPreview.innerHTML = rawHtmlContent;

  closeHtmlModal();
  toast('HTML aplicado na prévia.');
}
